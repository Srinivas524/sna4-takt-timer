...
(function () {
  ...
  // ═══════════════════════════════════════════════════════
  // SP INIT — Fetch item types at runtime
  // ═══════════════════════════════════════════════════════
  function initSharePoint() {
          ...
          GM_xmlhttpRequest({
            ...
            onload: function(res) {
              try {
                ...
                SP_LISTS[key].type = data.d.ListItemEntityTypeFullName;
                resolve();
              } catch(e) { reject(e); }
            },
    ...
    return Promise.all(promises).then(function() {
      SP_READY = true;
      console.log('SP: All item types loaded');
      return true;
    }).catch(function(err) {
      console.warn('SP Init failed:', err);
      SP_READY = false;
      return false;
    });
  }
  ...
  function dateFromSP(spDate) {
    if (!spDate) return null;
    return spDate.substring(0, 10);
  }
  ...
  function daysSince(dateStr) {
    var then = new Date(dateStr + 'T12:00:00Z');
    var now = new Date(getTodayStr() + 'T12:00:00Z');
        ...
        GM_xmlhttpRequest({
          ...
          onload: function(res) {
            try {
              ...
              if (result.error) { reject(new Error(result.error.message.value)); return; }
              resolve(result.d);
            } catch(e) { reject(e); }
          },
          onerror: function(err) { reject(err); }
  ...
  function spPatch(listKey, itemId, data) {
    return getDigest().then(function(digest) {
      return new Promise(function(resolve, reject) {
        ...
        GM_xmlhttpRequest({
          method: 'POST',
          url: spUrl(listKey) + '/items(' + itemId + ')',
          headers: {
            'Accept': 'application/json;odata=verbose',
            'Content-Type': 'application/json;odata=verbose',
            'X-RequestDigest': digest,
          ...
          onload: function(res) {
            if (res.status === 204 || res.status === 200) { resolve(true); }
              ...
              try {
                var result = JSON.parse(res.responseText);
                if (result.error) reject(new Error(result.error.message.value));
                else resolve(true);
  ...
  function spUpsert(listKey, titleKey, data) {
    return spGetByTitle(listKey, titleKey).then(function(existing) {
      if (existing) {
        return spPatch(listKey, existing.Id, data).then(function() {
          return { action: 'updated', id: existing.Id };
        });
      } else {
        data.Title = titleKey;
        return spPost(listKey, data).then(function(result) {
  ...
  function loadAllAssociates() {
    var filter = "IsActive eq 1";
    var select = 'Id,Title,AssociateName,CoachingNotes,IsActive,CreatedDate';
    return spGet('associates', filter, select).then(function(items) {
      return items.map(function(item) {
        return {
          ...
          createdDate: dateFromSP(item.CreatedDate) || getTodayStr()
        };
      });
    });
  }
  ...
  function spSaveAssociate(login, name, coachingNotes) {
    var data = {
      ...
      'IsActive': true,
      'CreatedDate': dateForSP(getTodayStr())
    };
    return spUpsert('associates', login, data);
  }
  ...
  function spSaveObservation(obsData) {
    ...
    var data = {
      'AssociateLogin': obsData.login,
      'ObsDate': dateForSP(obsData.date),
      'ProcessPath': obsData.process,
      'SubProcess': obsData.sub || '',
      'SetNumber': obsData.setNum,
      'ObsNumber': obsData.obsNum,
      'StartTime': obsData.startTime || '',
      'EndTime': obsData.endTime || '',
      'TaskTimesJSON': JSON.stringify(obsData.taskTimes || []),
      'TotalTime': obsData.totalTime || null,
      'TargetTotal': obsData.targetTotal || null,
      'AuditorLogin': obsData.auditorLogin || '',
      'AuditorName': obsData.auditorName || ''
    };
    return spUpsert('observations', key, data).then(function(result) {
      dataCache.clear();
      return recalcDailySummary(obsData.login, obsData.date, obsData.process, obsData.sub).then(function() {
        return result;
      });
    });
  }
  ...
  function spClearObservation(login, date, process, sub, setNum, obsNum) {
    ...
    return spGetByTitle('observations', key).then(function(existing) {
      if (existing) {
        return spPatch('observations', existing.Id, {
          'StartTime': '', 'EndTime': '', 'TaskTimesJSON': '[]', 'TotalTime': null
        }).then(function() {
          dataCache.clear();
  ...
  function recalcDailySummary(login, date, process, sub) {
    return loadObservationsForDay(login, date, process, sub).then(function(sets) {
      ...
      var totalSets = setNumbers.length;
      var auditors = {};
      ...
      for (var t = 0; t < allTotals.length; t++) {
        sum += allTotals[t];
        if (allTotals[t] < minVal) minVal = allTotals[t];
        if (allTotals[t] > maxVal) maxVal = allTotals[t];
      }
      ...
      if (allTaskArrays.length > 0) {
        var maxTasks = 0;
        for (var a = 0; a < allTaskArrays.length; a++) {
          if (allTaskArrays[a].length > maxTasks) maxTasks = allTaskArrays[a].length;
        }
        for (var ti = 0; ti < maxTasks; ti++) {
          var tSum = 0, tCount = 0;
          for (var ai = 0; ai < allTaskArrays.length; ai++) {
            if (allTaskArrays[ai][ti] !== undefined) { tSum += allTaskArrays[ai][ti]; tCount++; }
          }
          taskAvgs.push(tCount > 0 ? Math.round((tSum / tCount) * 100) / 100 : 0);
        }
      }

      var config = PROCESS_PATHS[process];
      var subConfig = config ? (config[sub] || config['_default']) : null;
      var targetTotal = subConfig ? subConfig.totalTarget : 0;
      var withinCount = 0;
      if (targetTotal > 0) {
        for (var w = 0; w < allTotals.length; w++) {
          if (allTotals[w] <= targetTotal) withinCount++;
        }
      }
      var withinPct = targetTotal > 0 ? Math.round((withinCount / totalObs) * 1000) / 10 : null;

      return spUpsert('dailySummaries', summaryKey, {
        'AssociateLogin': login, 'SummaryDate': dateForSP(date),
        'ProcessPath': process, 'SubProcess': sub || '',
        'TotalSets': totalSets, 'TotalObs': totalObs,
        'AvgTotal': avgTotal, 'MinTotal': minVal, 'MaxTotal': maxVal,
        'TaskAvgsJSON': JSON.stringify(taskAvgs), 'TargetTotal': targetTotal,
        'WithinTargetPct': withinPct, 'AuditorsJSON': JSON.stringify(Object.keys(auditors))
      });
    });
  }
  ...
  function loadDailySummary(login, date, process, sub) {
    ...
    return spGetByTitle('dailySummaries', key, 'Id,Title,TotalSets,TotalObs,AvgTotal,MinTotal,MaxTotal,TaskAvgsJSON,TargetTotal,WithinTargetPct,AuditorsJSON').then(function(item) {
      ...
      return {
        spId: item.Id, totalSets: item.TotalSets || 0, totalObs: item.TotalObs || 0,
        avgTotal: item.AvgTotal || 0, minTotal: item.MinTotal || 0, maxTotal: item.MaxTotal || 0,
        taskAvgs: taskAvgs, targetTotal: item.TargetTotal || 0,
        withinTargetPct: item.WithinTargetPct, auditors: auditors
      };
    });
  }
  ...
  function loadAssociateSummariesForDate(login, date) {
    ...
    return spGet('dailySummaries', filter, 'Id,Title,ProcessPath,SubProcess,TotalSets,TotalObs,AvgTotal,TargetTotal,WithinTargetPct').then(function(items) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        summaries[key] = {
          process: item.ProcessPath, sub: item.SubProcess || '_default',
          totalSets: item.TotalSets || 0, totalObs: item.TotalObs || 0,
          avgTotal: item.AvgTotal || 0, targetTotal: item.TargetTotal || 0,
          withinTargetPct: item.WithinTargetPct
        };
      }
      return summaries;
    });
  ...
  function computeProcessAverage(process, sub, date) {
    ...
    return spGet('dailySummaries', filter, 'AssociateLogin,TotalObs,AvgTotal,TaskAvgsJSON,TargetTotal,WithinTargetPct').then(function(items) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        target = items[i].TargetTotal || target;
  ...
  function computeHistoricalAvg(login, process, sub, daysBack) {
    ...
    return loadObservationsForRange(login, process, sub, startDate, endDate).then(function(items) {
      ...
      var cfg = PROCESS_PATHS[process]; var sc = cfg ? (cfg[sub] || cfg['_default']) : null; var tgt = sc ? sc.totalTarget : 0;
  ...
  function buildPickTasks(read, locate, move, drive) {
    ...
    return { tasks: tasks, totalTarget: read + locate + move + drive };
  }

  function buildPackTasks(t1, t2, t3, t4, t5, t6, t7, t8, t9) {
    var allTasks = [
      { name: "Scan cage", target: t1 }, { name: "Scan item / move item to station", target: t2 },
      { name: "Read screen", target: t3 }, { name: "Build box / tape if applicable", target: t4 },
      { name: "Place item into box", target: t5 }, { name: "P-Slip / labels", target: t6 },
      { name: "Dunnage", target: t7 }, { name: "Scan / add SPOO", target: t8 },
  ...
  function buildStowTasks(locate, openNet, grab, stow, confirm, drive) {
    var tasks = [
      { name: "Time to locate bin", target: locate }, { name: "Open cage netting", target: openNet },
      { name: "Grab item from cage", target: grab }, { name: "Stow unit into bin", target: stow },
      { name: "Confirm stow on scanner", target: confirm }, { name: "Drive time from bin to bin", target: drive }
    ];
    return { tasks: tasks, totalTarget: locate + openNet + grab + stow + confirm + drive };
  }
  ...
  function buildDockTasks() {
    ...
    return { tasks: tasks, totalTarget: 24, dockNote: '⚠ Includes possible waterspider @ 100 UPH' };
  }
  ...
  function hasSubPaths(process) {
    var subs = Object.keys(PROCESS_PATHS[process]);
    return !(subs.length === 1 && subs[0] === '_default');
  }

  function getConfig(process, sub) {
    return PROCESS_PATHS[process][sub] || PROCESS_PATHS[process]['_default'];
  }
  ...
  var state = {
    ...
    selectedProcess: 'Pick',
    selectedSubProcess: 'Singles',
    selectedObs: null,
    currentSet: 1,
    isRunning: false,
    currentTaskIndex: -1,
    ...
    // Loaded data (from SP)
    associates: [],
    sets: {},              // loaded observation sets for current view
    daySummaries: {},      // loaded summaries for current associate+date
    processAvg: null,      // loaded process average for current view
    historicalAvg: null     // loaded historical average
  };
  ...
  function getCurrentAssociate() {
    if (state.currentAssociateIndex >= 0 && state.currentAssociateIndex < state.associates.length) {
      return state.associates[state.currentAssociateIndex];
    }
    return null;
  }
  ...
  function formatTime(date) {
    ...
    var cs = Math.floor(date.getMilliseconds() / 100);
    return h + ':' + m + ':' + s + '.' + cs;
  }
  ...
  function formatElapsed(ms) {
    ...
    var cs = Math.floor((ms % 1000) / 10).toString().padStart(2,'0');
    return min + ':' + sec + '.' + cs;
  }
  ...
  function saveAuditorLocally() {
    try { localStorage.setItem('sna4_auditor', JSON.stringify(auditorInfo)); } catch(e) {}
  }

  function loadAuditorLocally() {
    try {
      var raw = localStorage.getItem('sna4_auditor');
      if (raw) { var p = JSON.parse(raw); auditorInfo.name = p.auditorName || p.name || ''; auditorInfo.login = p.auditorLogin || p.login || ''; }
    } catch(e) {}
  }
  ...
  function isSetLocked(setData) {
    // A set is locked if all 5 obs are complete
    if (!setData) return false;
    return setData.isComplete === true;
  }

  function canEditCurrentSet() {
    // Can edit only if: today + set is not complete
    if (isReadOnly()) return false;
    var setData = state.sets[state.currentSet];
    if (!setData) return true; // empty set = editable
    return !setData.isComplete;
  }
  // ═══════════════════════════════════════════════════════
  // STYLES
  // ═══════════════════════════════════════════════════════
  ...
  styles.textContent = `
    ...
    .takt-day-nav {
      ...
      padding: 8px 24px; background: linear-gradient(135deg, #f0f9ff, #e0f2fe);
      border-bottom: 2px solid #7dd3fc; flex-shrink: 0;
    }
    .takt-day-btn {
      width: 34px; height: 34px; border-radius: 8px; border: 2px solid #7dd3fc;
      background: white; color: #0284c7; font-size: 16px; font-weight: 800;
    ...
    .takt-day-today-btn {
      padding: 5px 14px; border-radius: 8px; border: 2px solid #7dd3fc;
      background: white; color: #0284c7; font-size: 11px; font-weight: 700;
    ...
    .takt-day-readonly-badge {
      padding: 4px 12px; border-radius: 6px; background: #fef3c7; color: #92400e;
    ...
    .takt-assoc-card {
      ...
      border: 2px solid #86efac; min-width: 0; transition: all 0.3s;
    }
    .takt-assoc-avatar {
    ...
    .takt-set-bar {
      ...
      background: #fdf4ff; border-bottom: 2px solid #e879f9; flex-shrink: 0;
    }
    .takt-set-label { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; color: #a21caf; }
    .takt-set-pills { display: flex; gap: 5px; }
    .takt-set-pill {
      padding: 6px 14px; border-radius: 8px; border: 2px solid #e879f9;
      background: white; color: #a21caf; font-size: 12px; font-weight: 600;
      cursor: pointer; transition: all 0.2s; font-family: 'Inter', sans-serif; position: relative;
    }
    ...
    .takt-set-add-btn {
      padding: 6px 14px; border-radius: 8px; border: 2px dashed #e879f9;
      background: white; color: #a21caf; font-size: 12px; font-weight: 700;
      cursor: pointer; transition: all 0.2s; font-family: 'Inter', sans-serif;
    }
    ...
    .takt-control-bar {
      display: flex; align-items: center; gap: 10px; padding: 10px 24px;
      background: #f8fafc; border-bottom: 1px solid #e2e8f0; flex-shrink: 0; flex-wrap: wrap;
    }
  ...
  function renderPanel() {
    ...
    var dayNavHTML = '<div class="takt-day-nav">'
      + '<button class="takt-day-btn" id="takt-day-prev" title="Previous day">‹</button>'
      + '<div class="takt-day-label"><span class="day-name">' + dayLabel + '</span> — ' + formatDateDisplay(state.currentDate) + '</div>'
      + '<button class="takt-day-btn" id="takt-day-next"' + futureDisabled + ' title="Next day">›</button>'
    ...
    if (assoc) {
      ...
      var statsLine = '';
      if (histAvg) {
        statsLine = '📊 Avg: ' + histAvg.avgTotal + 's';
        if (histAvg.targetTotal > 0) {
          statsLine += histAvg.avgTotal <= histAvg.targetTotal ? ' ✅' : ' ⚠️';
        }
    ...
    var footerHTML = '<div class="takt-footer"><div class="takt-footer-left">'
      ...
      + '</div><div class="takt-footer-status">' + state.associates.length + ' associate(s) | ' + getDayLabel(state.currentDate) + '</div></div>';

    // No associate — empty state
    if (!assoc) {
  ...
  function renderSummaryView(headerHTML, loadingHTML, dayNavHTML, auditorBarHTML, assocBarHTML, footerHTML) {
    ...
    Object.keys(PROCESS_PATHS).forEach(function(process) {
      var subs = PROCESS_PATHS[process];
      ...
      subKeys.forEach(function(sub) {
        ...
        var totalObs = summary ? summary.totalObs : 0;
        var avgTotal = summary ? summary.avgTotal : 0;
        var targetTotal = summary ? summary.targetTotal : 0;
        var isDefault = sub === '_default';
        var subLabel = isDefault ? process : sub;

        var pct = totalObs > 0 ? Math.min((totalObs / 5) * 100, 100) : 0;
        var isDone = totalObs >= 5;
        var isEmpty = totalObs === 0;
        ...
        if (totalObs > 0) {
          avgText = avgTotal + 's';
          if (targetTotal > 0) {
            avgClass = avgTotal <= targetTotal ? ' good' : ' over';
            avgText += ' / ' + targetTotal + 's';
          }
        }
  ...
  function renderTableView(headerHTML, loadingHTML, dayNavHTML, auditorBarHTML, assocBarHTML, footerHTML) {
    ...
    var TOTAL_TARGET = config.totalTarget;
    var showTargets = TOTAL_TARGET > 0;
    var sets = state.sets || {};
    ...
    var setLocked = isSetLocked(currentSetData);
    var canEdit = canEditCurrentSet();
    var obs = state.selectedObs ? (observations[state.selectedObs] || null) : null;
    var tasksDone = obs ? obs.taskTimes.length : 0;
    var totalTasks = TASKS.length;
    var progress = (tasksDone / totalTasks) * 100;
    var isComplete = obs && obs.totalTime !== null;
    ...
    if (setKeys.length === 0) {
      setBarHTML += '<span class="takt-set-pill selected">Set 1</span>';
    } else {
      for (var si = 0; si < setKeys.length; si++) {
        ...
        setBarHTML += '<button class="takt-set-pill' + sCls + '" data-set="' + sn + '"' + sDis + '>Set ' + sn + '</button>';
      }
    }
    setBarHTML += '</div>';
    ...
    for (var ci = 0; ci < setKeys.length; ci++) {
      if (!sets[setKeys[ci]].isComplete) { allSetsComplete = false; break; }
    }
    var showAddSet = canAddSet && (setKeys.length === 0 || allSetsComplete);
    if (showAddSet && setKeys.length > 0) {
      setBarHTML += '<button class="takt-set-add-btn" id="takt-add-set">＋ Add New Set</button>';
    }
    setBarHTML += '</div>';
    ...
    for (var pi = 1; pi <= NUM_OBS; pi++) {
      var obsIsSel = state.selectedObs === pi;
      var obsData = observations[pi];
      var obsDone = obsData && obsData.totalTime !== null;
    ...
    // Start/Record button
    var btnClass = 'start-btn';
    var btnText = '▶ START';
    var btnDisabled = !state.selectedObs || isComplete || !canEdit;
    if (state.isRunning && state.currentTaskIndex === -1) {
      btnClass = 'recording-btn'; btnText = '⏱ CLICK — Record Start Time'; btnDisabled = false;
    } else if (state.isRunning) {
      btnClass = 'recording-btn'; btnText = '⏱ CLICK — Record Task ' + (state.currentTaskIndex + 1) + '/' + totalTasks; btnDisabled = false;
    }
    ...
    var processAvg = state.processAvg;
    ...
    for (var st = 1; st <= NUM_OBS; st++) {
      var stObs = observations[st];
      var stVal = stObs ? stObs.startTime : null;
      var stActive = state.selectedObs === st;
    ...
    TASKS.forEach(function(task, idx) {
      ...
      tableRowsHTML += '<td style="padding-left:' + (isCurrentTask ? '20px' : '24px') + ';"><span style="color:#94a3b8;font-size:10px;font-weight:700;margin-right:6px;">' + String(idx+1).padStart(2,'0') + '</span>' + escapeHtml(task.name) + '</td>';
      tableRowsHTML += '<td class="target-col">' + (task.target > 0 ? task.target + 's' : 'N/A') + '</td>';

      for (var oi = 1; oi <= NUM_OBS; oi++) {
        var oData = observations[oi];
        var isA = state.selectedObs === oi;
        var val = oData ? oData.taskTimes[idx] : undefined;
      ...
      // Daily avg for this task
      if (hasDaily && dailySummary.taskAvgs[idx] !== undefined) {
        var dAvg = dailySummary.taskAvgs[idx];
        var dCls = task.target > 0 ? (dAvg <= task.target ? 'good' : 'over') : '';
        tableRowsHTML += '<td class="avg-cell ' + dCls + '">' + dAvg + 's</td>';
      } else if (hasDaily) {
        tableRowsHTML += '<td class="avg-cell">—</td>';
      }

      // Process avg for this task
      if (hasProcess && processAvg.taskAvgs[idx] !== undefined) {
        var pAvg = processAvg.taskAvgs[idx];
        tableRowsHTML += '<td class="pavg-cell">' + pAvg + 's</td>';
      } else if (hasProcess) {
        tableRowsHTML += '<td class="pavg-cell">—</td>';
      }
    ...
    for (var et = 1; et <= NUM_OBS; et++) {
      var etObs = observations[et];
      var etVal = etObs ? etObs.endTime : null;
      var etActive = state.selectedObs === et;
    ...
    if (hasProcess) {
      tableRowsHTML += '<td class="pavg-cell">' + processAvg.overallAvgTotal + 's</td>';
    }
    tableRowsHTML += '</tr>';
    ...
    // Coaching section
    var coachingCollapsed = !state.coachingExpanded;
    var coachingDisabled = readOnly ? ' disabled' : '';
  ...
  // ═══════════════════════════════════════════════════════
  // WIRE EVENTS — BASE (shared across views)
  // ═══════════════════════════════════════════════════════
  function wireBaseEvents() {
    var closeBtn = document.getElementById('takt-close');
    var minBtn = document.getElementById('takt-minimize');
    if (closeBtn) closeBtn.onclick = togglePanel;
    if (minBtn) minBtn.onclick = togglePanel;

    // Auditor inputs
    var audName = document.getElementById('takt-auditor-name');
    var audLogin = document.getElementById('takt-auditor-login');
    ...
    // Day navigation
    var dayPrev = document.getElementById('takt-day-prev');
    var dayNext = document.getElementById('takt-day-next');
    var dayToday = document.getElementById('takt-day-today');
    if (dayPrev) dayPrev.onclick = function() {
      if (state.isRunning) return;
      state.currentDate = addDays(state.currentDate, -1);
      state.selectedObs = null; state.currentSet = 1; state.view = 'summary';
      reloadCurrentView();
    };
    if (dayNext) dayNext.onclick = function() {
      if (state.isRunning || isToday(state.currentDate)) return;
      state.currentDate = addDays(state.currentDate, 1);
      state.selectedObs = null; state.currentSet = 1; state.view = 'summary';
      reloadCurrentView();
    };
    if (dayToday) dayToday.onclick = function() {
      if (state.isRunning || isToday(state.currentDate)) return;
    ...
    var prevBtn = document.getElementById('takt-nav-prev');
    var nextBtn = document.getElementById('takt-nav-next');
    if (prevBtn) prevBtn.onclick = function() { navigateAssociate(-1); };
    if (nextBtn) nextBtn.onclick = function() { navigateAssociate(1); };

    // Associate actions
    var searchBtn = document.getElementById('takt-search-assoc');
    var addBtn = document.getElementById('takt-add-assoc');
    var emptyAddBtn = document.getElementById('takt-empty-add');
    var deleteBtn = document.getElementById('takt-delete-assoc');
    ...
    // Footer
    var exportBtn = document.getElementById('takt-export-csv');
    var copyBtn = document.getElementById('takt-copy-data');
    var clearAllBtn = document.getElementById('takt-clear-all');
  ...
  function wireTableEvents() {
    ...
    if (backBtn) backBtn.onclick = function() {
      if (state.isRunning) return;
      state.view = 'summary'; state.selectedObs = null;
      reloadCurrentView();
    };
    ...
    panel.querySelectorAll('.takt-set-pill').forEach(function(btn) {
      btn.onclick = function() {
        if (state.isRunning) return;
        state.currentSet = parseInt(btn.dataset.set);
        state.selectedObs = null;
        renderPanel();
      };
    });
    ...
    if (addSetBtn) addSetBtn.onclick = function() {
      ...
      getNextSetNumber(assoc.login, state.currentDate, state.selectedProcess, state.selectedSubProcess).then(function(nextSet) {
        ...
        state.selectedObs = null;
        state.sets[nextSet] = { setNumber: nextSet, observations: {}, isComplete: false, auditorLogin: '', auditorName: '' };
        renderPanel();
        showToast('＋ Set ' + nextSet + ' created');
      });
    };
    ...
    // Start/Record button
    var startBtn = document.getElementById('takt-start-btn');
    if (startBtn) startBtn.onclick = handleStartStop;

    // Clear button
    var clearBtn = document.getElementById('takt-clear-btn');
    if (clearBtn) clearBtn.onclick = handleClear;

    // Coaching
    var coachToggle = document.getElementById('takt-coaching-toggle');
    var coachNotes = document.getElementById('takt-coaching-notes');
      ...
      coachNotes.oninput = function(e) {
        ...
        if (assoc) {
          ...
          coachTimer = setTimeout(function() {
            spUpdateCoachingNotes(assoc.login, e.target.value).catch(function() {});
          }, 1500);
        }
      };
    }
  ...
  function handleStartStop() {
    ...
    // Initialize obs if it doesn't exist
    if (!obs) {
      ...
      if (!currentSetData) {
        state.sets[state.currentSet] = { setNumber: state.currentSet, observations: {}, isComplete: false, auditorLogin: '', auditorName: '' };
    ...
    if (!state.isRunning && obs.totalTime === null) {
      ...
      // Capture the date when START is clicked
      state.currentDate = getTodayStr();
    ...
    if (state.isRunning && state.currentTaskIndex >= 0) {
      var now = Date.now();
      var elapsed = Math.round((now - state.lastClickTime) / 1000);
      obs.taskTimes.push(elapsed);
      ...
      if (obs.taskTimes.length >= TASKS.length) {
        ...
        obs.targetTotal = TOTAL_TARGET;
        obs.auditorLogin = auditorInfo.login;
        obs.auditorName = auditorInfo.name;
        ...
        var doneCount = 0;
        for (var oi = 1; oi <= NUM_OBS; oi++) {
          var o = currentSetData.observations[oi];
          if (o && o.totalTime !== null) doneCount++;
        }
        ...
        spSaveObservation({
          ...
          startTime: obs.startTime, endTime: obs.endTime,
          taskTimes: obs.taskTimes, totalTime: obs.totalTime,
          targetTotal: TOTAL_TARGET,
          auditorLogin: auditorInfo.login, auditorName: auditorInfo.name
        }).then(function() {
          state.loading = false;
          // Reload daily summary and process avg
          return Promise.all([
          ...
          state.dailySummary = results[0];
          state.processAvg = results[1];
          renderPanel();
        }).catch(function(err) {
          state.loading = false;
        ...
        if (showTargets) {
          ...
          showToast('✅ Obs ' + state.selectedObs + ' complete! ' + obs.totalTime + 's total');
        }
        if (currentSetData.isComplete) {
          showToast('🔒 Set ' + state.currentSet + ' complete and locked!');
        }
      } else {
        state.currentTaskIndex = obs.taskTimes.length;
        renderPanel();
      }
    }
  }
  ...
  function startElapsedTimer() {
    ...
    state.elapsedInterval = setInterval(function() {
      var timerEl = document.getElementById('takt-timer-main');
      var cellEl = document.getElementById('takt-live-cell');
  ...
  function handleClear() {
    ...
    showConfirm('Clear Observation ' + state.selectedObs + '?', 'All recorded times for this observation will be deleted.', function() {
      ...
      if (currentSetData && currentSetData.observations[state.selectedObs]) {
        currentSetData.observations[state.selectedObs] = {
          obsNumber: state.selectedObs, startTime: null, endTime: null,
          taskTimes: [], totalTime: null, targetTotal: null, auditorLogin: '', auditorName: ''
        };
      ...
      spClearObservation(assoc.login, state.currentDate, state.selectedProcess, state.selectedSubProcess, state.currentSet, state.selectedObs).then(function() {
        ...
        renderPanel();
      }).catch(function(err) { console.warn('Clear failed:', err); });
    });
  }
  ...
  function handleClearAll() {
    ...
    showConfirm('Clear ALL Data?', 'This will deactivate all associates. Observation history is preserved in SharePoint.', function() {
      state.isRunning = false; state.currentTaskIndex = -1;
      fab.classList.remove('active'); stopElapsedTimer();
      var promises = state.associates.map(function(a) { return spDeactivateAssociate(a.login); });
      Promise.all(promises).then(function() {
        state.associates = []; state.currentAssociateIndex = -1;
        state.selectedObs = null; state.view = 'summary';
        renderPanel();
        showToast('🗑 All associates deactivated');
      });
    });
  }
  ...
  function navigateAssociate(direction) {
    if (state.isRunning) return;
    var len = state.associates.length;
    if (len === 0) return;
  ...
  function showSearchOverlay() {
    ...
    var bar = document.getElementById('takt-associate-bar');
    if (!bar) return;
    ...
    function buildResults(query) {
      ...
      var filtered = state.associates.filter(function(a) {
        return a.name.toLowerCase().indexOf(q) > -1 || a.login.toLowerCase().indexOf(q) > -1;
      });
      if (filtered.length === 0) return '<div class="takt-search-no-results">No associates found</div>';
      return filtered.map(function(a) {
        var realIdx = state.associates.indexOf(a);
        var isActive = realIdx === state.currentAssociateIndex;
    ...
    var input = document.getElementById('takt-search-input');
    var resultsContainer = document.getElementById('takt-search-results');
    input.focus();
    input.oninput = function() { resultsContainer.innerHTML = buildResults(input.value); wireSearchResults(); };
    ...
    function wireSearchResults() {
      resultsContainer.querySelectorAll('.takt-search-result').forEach(function(el) {
        el.onclick = function() {
          ...
          overlay.remove();
          reloadCurrentView();
          showToast('👤 Switched to ' + state.associates[parseInt(el.dataset.index)].name);
        };
      });
    }
    wireSearchResults();
    ...
    setTimeout(function() {
      var closeHandler = function(e) {
        if (!overlay.contains(e.target) && e.target.id !== 'takt-search-assoc') {
          overlay.remove(); document.removeEventListener('click', closeHandler);
        }
      };
  ...
  function showAddForm(prefillName) {
    var existing = document.querySelector('.takt-add-overlay');
    ...
    overlay.innerHTML = '<div class="takt-add-form">'
      ...
      + '<div class="takt-add-field"><label>Associate Name</label>'
      ...
      + '<div class="takt-add-field"><label>Associate Login</label>'
    ...
    var nameInput = document.getElementById('takt-add-name');
    var loginInput = document.getElementById('takt-add-login');
    var submitBtn = document.getElementById('takt-add-submit');
    var nameWarn = document.getElementById('takt-name-warn');
    var loginWarn = document.getElementById('takt-login-warn');
    nameInput.focus();
    ...
    submitBtn.onclick = function() {
      ...
      spSaveAssociate(login, name, '').then(function() {
        ...
        state.associates.push(newAssoc);
        state.currentAssociateIndex = state.associates.length - 1;
        state.selectedObs = null; state.currentSet = 1; state.view = 'summary';
        overlay.remove();
        reloadCurrentView();
        ...
        submitBtn.textContent = 'Add Associate';
  ...
  // ═══════════════════════════════════════════════════════
  function initDrag() {
    var handle = document.getElementById('takt-drag-handle');
    if (!handle) return;
  ...
  function exportCSV() {
    ...
    csv += 'Associate,' + assoc.name + '\nAssoc Login,' + assoc.login + '\n';
    csv += 'Process,' + state.selectedProcess + '\n';
  ...
  function copyData() {
    ...
    for (var i = 1; i <= NUM_OBS; i++) {
      ...
      if (!o || o.taskTimes.length === 0) continue;
      text += '── Obs ' + i + ' ──\nStart: ' + (o.startTime || 'N/A') + '\n';
      TASKS.forEach(function(t, idx) {
        var v = o.taskTimes[idx];
        if (v !== undefined) text += '  ' + (t.target > 0 ? (v <= t.target ? '✅' : '⚠️') : '⏱') + ' ' + t.name + ': ' + v + 's\n';
      });
  ...
  // ═══════════════════════════════════════════════════════
  // DATA LOADING — Async orchestration
  // ═══════════════════════════════════════════════════════
  function reloadCurrentView() {
    state.loading = true;
    renderPanel(); // show loading state

    var assoc = getCurrentAssociate();
    if (!assoc) { state.loading = false; renderPanel(); return; }

    var promises = [
      loadAssociateSummariesForDate(assoc.login, state.currentDate)
    ];

    // Load historical avg for associate card
    ...
    Promise.all(promises).then(function(results) {
      state.daySummaries = results[0];
      state.historicalAvg = results[1];
      state.loading = false;
  ...
  function loadTableData() {
    ...
    Promise.all([
      ...
      state.dailySummary = results[1];
      state.processAvg = results[2];
      state.historicalAvg = results[3];

      // Set currentSet to first available
      var setKeys = Object.keys(state.sets).sort(function(a,b) { return parseInt(a) - parseInt(b); });
      if (setKeys.length > 0) {
        // If current set doesn't exist in loaded data, pick the last one
        if (!state.sets[state.currentSet]) {
          state.currentSet = parseInt(setKeys[setKeys.length - 1]);
        }
      } else {
        state.currentSet = 1;
      }
  ...
  initSharePoint().then(function(ready) {
    if (ready) {
      ...
      // Load associates
      return loadAllAssociates();
    } else {
      console.warn('⚠ SNA4 Takt Timer — SharePoint offline, limited functionality');
      return [];
    }
